...
func (p *LiqoProvider) CreatePod(ctx context.Context, homePod *corev1.Pod) error {
	...
	foreignObj, err := forge.HomeToForeign(ctx, homePod, nil, forge.LiqoOutgoingKey)
	if err != nil {
...
func (p *LiqoProvider) DeletePod(ctx context.Context, pod *corev1.Pod) (err error) {
	...
	if value, ok := vkContext.CallingFunction(ctx); ok && value == vkContext.DeleteDanglingPods {
		...
		foreignNamespace, err = p.namespaceMapper.NatNamespace(ctx, pod.Namespace)
		if err != nil {
			return err
		}
	}
...
func (p *LiqoProvider) GetPod(ctx context.Context, namespace, name string) (pod *corev1.Pod, err error) {
	...
	foreignNamespace, err := p.namespaceMapper.NatNamespace(ctx, namespace)
	if err != nil {
...
func (p *LiqoProvider) GetPods(ctx context.Context) ([]*corev1.Pod, error) {
	...
	namespacesMap, err := p.namespaceMapper.MappedNamespaces(ctx)
	if err != nil {
		return nil, err
	}
	for _, foreignNamespace := range namespacesMap {
		...
		for _, pod := range pods {
			homePod, err := forge.ForeignToHome(ctx, pod.(*corev1.Pod), nil, forge.LiqoNodeName())
			if err != nil {
				return nil, err
			}
...
func (p *LiqoProvider) RunInContainer(ctx context.Context, homeNamespace, homePodName, containerName string,
	...
	foreignNamespace, err := p.namespaceMapper.NatNamespace(ctx, homeNamespace)
	if err != nil {
		return err
	}
...
func (p *LiqoProvider) GetStatsSummary(ctx context.Context) (*stats.Summary, error) {
	...
	for home, foreign := range namespacesMap {
