...
func (npr *NamespacedPodReflector) Handle(ctx context.Context, name string) error {
	...
	// The local pod does no longer exist. Ensure it is also absent from the remote cluster.
	if !localExists {
		defer tracer.Step("Ensured the absence of the remote object")
		if !kerrors.IsNotFound(rerr) {
			klog.V(4).Infof("Deleting remote pod %q, since local %q does no longer exist", npr.RemoteRef(name), npr.LocalRef(name))
			return npr.DeleteRemote(ctx, npr.remotePodsClient, PodReflectorName, name, remote.GetUID())
		}

		klog.V(4).Infof("Local pod %q and remote pod %q both vanished", npr.LocalRef(name), npr.RemoteRef(name))
		return nil
	}
	...
	if !local.DeletionTimestamp.IsZero() {
		...
		if !remoteExists {
			...
			klog.V(4).Infof("Deleting terminating local pod %q, since remote %q does no longer exist", npr.RemoteRef(name), npr.LocalRef(name))
		...
		if remote.DeletionTimestamp.IsZero() {
			tracer.Step("Ensured the absence of the remote object")
			klog.V(4).Infof("Deleting remote pod %q, since local %q is terminating", npr.RemoteRef(name), npr.LocalRef(name))
			return npr.DeleteRemote(ctx, npr.remotePodsClient, PodReflectorName, name, remote.GetUID())
		}
	...
	pod := forge.RemotePod(local, remote, npr.RemoteNamespace())
	...
	if !remoteExists {
		...
		if _, err := npr.remotePodsClient.Create(ctx, pod, metav1.CreateOptions{FieldManager: forge.ReflectionFieldManager}); err != nil {
			if kerrors.IsAlreadyExists(err) {
				klog.Infof("Remote pod %q already exists (local: %q)", npr.RemoteRef(name), npr.LocalRef(name))
				return nil
			}
			klog.Errorf("Failed to create remote pod %q (local: %q): %v", npr.RemoteRef(name), npr.LocalRef(name), err)
			return err
		}

		klog.Infof("Remote pod %q successfully created (local: %q)", npr.RemoteRef(name), npr.LocalRef(name))
		tracer.Step("Created the remote pod")
		// Whatever the outcome, here we return without updating the status, as it will be triggered by future events.
		return nil
	}

	// Otherwise update the remote pod, if necessary.
	// TODO: fix the update logic when moving to the shadowpod and having refactored the forge logic.
	// if !reflect.DeepEqual(remote.ObjectMeta, pod.ObjectMeta) || !reflect.DeepEqual(remote.Spec, pod.Spec) {
	// 	 remote, rerr = npr.remotePodsClient.Update(ctx, pod, metav1.UpdateOptions{FieldManager: forge.ReflectionFieldManager})
	// 	 if rerr != nil {
	// 	 	klog.Errorf("Failed to update remote pod %q (local: %q): %v", npr.RemoteRef(name), npr.LocalRef(name), rerr)
	// 	 	return rerr
	// 	 }
	//
	// 	 klog.Infof("Remote pod %q successfully updated (local: %q)", npr.RemoteRef(name), npr.LocalRef(name))
	// 	 tracer.Step("Updated the remote pod")
	// }
...
func (npr *NamespacedPodReflector) HandleStatus(ctx context.Context, local, remote *corev1.Pod) error {
	...
	pod := forge.LocalPod(local, remote, translator, info.Restarts)
	...
	if reflect.DeepEqual(local.Status, pod.Status) {
	...
	_, err := npr.localPodsClient.UpdateStatus(ctx, pod, metav1.UpdateOptions{FieldManager: forge.ReflectionFieldManager})
	if err != nil {
...
// Exec executes a command in a container of a reflected pod.
func (npr *NamespacedPodReflector) Exec(ctx context.Context, pod, container string, cmd []string, attach api.AttachIO) error {
	klog.V(4).Infof("Requested to exec command in container %q of local pod %q (remote %q)", container, npr.LocalRef(pod), npr.RemoteRef(pod))
	...
	if err != nil {
		klog.Errorf("Failed to exec command in container %q of local pod %q (remote %q): %v", container, npr.LocalRef(pod), npr.RemoteRef(pod), err)
	...
	klog.Infof("Command in container %q in local pod %q (remote %q) successfully executed", container, npr.LocalRef(pod), npr.RemoteRef(pod))
	return nil
}

// Logs retrieves the logs of a container of a reflected pod.
func (npr *NamespacedPodReflector) Logs(ctx context.Context, pod, container string, opts api.ContainerLogOpts) (io.ReadCloser, error) {
	klog.V(4).Infof("Requested logs of container %q of local pod %q (remote %q)", container, npr.LocalRef(pod), npr.RemoteRef(pod))
	...
	stream, err := npr.remotePodsClient.GetLogs(pod, &logOpts).Stream(ctx)
	if err != nil {
		klog.Errorf("Failed to retrieve logs of container %q of local pod %q (remote %q): %v", container, npr.LocalRef(pod), npr.RemoteRef(pod), err)
		return nil, fmt.Errorf("could not get stream from logs request: %w", err)
	}
	klog.Infof("Logs of container %q of local pod %q (remote %q) successfully retrieved", container, npr.LocalRef(pod), npr.RemoteRef(pod))
	return stream, nil
}
...
func (npr *NamespacedPodReflector) RetrievePodInfo(pod string) *PodInfo {
	info, _ := npr.pods.LoadOrStore(pod, &PodInfo{})
	return info.(*PodInfo)
}

// ForgetPodInfo forgets about a pod and deletes the cached information.
func (npr *NamespacedPodReflector) ForgetPodInfo(pod string) {
	npr.pods.Delete(pod)
}
