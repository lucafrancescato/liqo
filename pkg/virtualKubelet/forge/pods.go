...
// LocalPod forges the status of the local pod, given the remote one.
...
// RemotePod forges the reflected pod, given the local one.
func RemotePod(local, remote *corev1.Pod, targetNamespace string) *corev1.Pod {
	if remote == nil {
		// The remote is nil if not already created.
		remote = &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: local.GetName(), Namespace: targetNamespace}}
	}

	return &corev1.Pod{
		ObjectMeta: RemoteObjectMeta(&local.ObjectMeta, &remote.ObjectMeta),
		Spec:       RemotePodSpec(local.Spec.DeepCopy(), remote.Spec.DeepCopy()),
	}
}
