...
var _ = Describe("Pod forging", func() {
	...
	Describe("the RemotePod function", func() {
		var local, remote, output *corev1.Pod
		...
		JustBeforeEach(func() { output = forge.RemotePod(local, remote, "remote-namespace") })
		...
		Context("the remote pod does not exist", func() {
			...
			It("should correctly reflect the pod spec", func() {
				...
				Expect(output.Spec.TerminationGracePeriodSeconds).To(PointTo(BeNumerically("==", 15)))
			})
		})

		Context("the remote pod already exists", func() {
			BeforeEach(func() {
				remote = &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "remote-name", Namespace: "remote-namespace", UID: "remote-uid"}}
